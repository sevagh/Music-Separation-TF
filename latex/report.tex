...
where $g(\cdot)$ are the time-shifted, localized windows, $X_{m}(f)$ is the DFT of the audio signal centered about time $mR$, and $R$ is the hop size between successive time-shifts of the window. Note how similar equations (1) and (2) are, which is expected since the original Gabor transform is the STFT with a Gaussian window. Practically, the STFT allows the use of different windows and overlap sizes \cite{stftinvertible}, as long as overlap-add conditions are respected.\footnote{\href{https://www.mathworks.com/help/signal/ref/iscola.html}{https://www.mathworks.com/help/signal/ref/iscola.html}}
...
The glockenspiel signal is well-known and useful to demonstrate transient and tonal properties of a musical signal -- it is available in the LTFAT\footnote{\href{https://ltfat.github.io/doc/signals/gspi.html}{https://ltfat.github.io/doc/signals/gspi.html}}, loaded by the \Verb#gspi# function. Figure \ref{fig:glockwaveform} shows the time-domain waveform:
...
Let's first consider the STFT. The standard \Verb#spectrogram# in MATLAB\footnote{\href{https://www.mathworks.com/help/signal/ref/spectrogram.html}{https://www.mathworks.com/help/signal/ref/spectrogram.html}} is computed using the short-time Fourier transform. The number of DFT output bins is set to twice the window size, and the overlap is set to half of the window size. An STFT with 3 different window sizes is shown in figure \ref{fig:glockspecs}.

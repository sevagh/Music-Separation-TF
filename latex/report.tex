...
The separation of tonal from transient is done as follows:

\begin{tight_enumerate}
	\item
		Initialize a random white noise signal.
	\item
		Within a supertile, compare the entropy of the signal being studied with the random white noise, considering the tonal/wide window transform coefficients. In the case where the entropy of the studied signal is higher than the random white noise by a threshold, set those coefficients to zero, resulting in a separation into tonal and residual components, and removal of transient components.
	\item
	...
	\item
		The above steps can be repeated for the transient/narrow window transform coefficients to separate the transient sound.
\end{tight_enumerate}

In the WMDCTLasso algorithm \cite{wmdct}, Group-LASSO (Least Absolute Shrinkage and Selection Operator) is used to separate tonal and transient sounds by grouping transform coefficients by sparsity. To describe the usefulness of Group-LASSO, \citet{sparsitykowalski} first state the problem of tonal + transient + noise decomposition as follows:
...
\section{Deep learning source separation with the NSGT}
\label{appendix:ml}

\citet{grais2017single} presented a deep learning architecture for performing single-channel source separation based on the Convolutional Denoising Auto-Encoder architecture. The architecture of the neural network is shown in \ref{fig:cdae_arch}.

\begin{figure}[ht]
	\centering
	\includegraphics[width=16cm]{./cdae_arch.png}
	\caption{Architecture of a Convolutional Denoising Auto-Encoder (CDAE)}
	\label{fig:cdae_arch}
\end{figure}

Their proposed system will use $N$ CDAEs for $N$ sources. The CDAE is trained on 15 frames of magnitude spectrograms with 1025 frequency bins ($\text{window\_size} = 1024, \text{nfft} = 2048$) -- at a sampling rate of 44100 Hz, this represents 0.348s of audio in $15 \times 1025 = 15375$ coefficients. The input of the CDAE is the magnitude of the mixed signal, and the learned output is the target source; the training data for a CDAE requires the mixed song as well as recordings of each individual source.

The perform the full source separation, the mixed signal is split into windowed segments of size 1024, and the spectrogram is taken every 15 frames to create a $15 \times 1025$ spectrogram. The magnitude spectrogram is passed into the CDAE, which computes the estimated magnitude spectrogram of the desired source. Then, the complex spectrogram of the desired source is created by combining the estimated magnitude and phase of the original mixture -- finally, the complex spectrogram is inverted to create 15 frames of separated signal for the desired source.

In figure \ref{fig:mixin_arch}, we show the block diagram of a source separation system adapted the one-pass HPSS algorithm to replace median filtering with trained CDAEs. This is a departure from the CDAE paper, which proposes to use the magnitude of the CDAE output and phase of the original mixture to invert into the separated sources. Instead, as in the HPSS framework, we can use the source magnitude estimates to compute soft masks to apply to the original mix. This performs much better in practice than the inversion strategy using the original mixture phase. Another major change is to replace the STFT with the NSGT using the Bark scale. This idea originated from two motivations:

\begin{tight_itemize}
	\item
		In line with this report, I wanted to show that the NSGT can replace a linear STFT in any kind of system. In the main body of this report, the CQT based on the NSGT was successfully substituted in DSP algorithms for source separation. In this case, the goal is to show that the NSGT can be used in machine learning algorithms based on the STFT.
	\item
		\citet{bfcc1, bfcc2} both report that Bark bands are useful for percussive content, and one of my pet obsessions is to create a high quality drum separator. The NSGT was configured to output 96 bins on the Bark frequency scale between 0 and 22050 Hz.
\end{tight_itemize}

Instead of splitting the input signal into magnitude spectrograms taken from 15 consecutive frames of size 1024, the input signal is split into 1-second chunks, and the 96-bin Bark-scale NSGT is taken; the magnitude NSGT for the 1 second chunks contains $1948 \times 96 = 187008$ coefficients. The final proposed system is named MiXiN, and is described in figure \ref{fig:mixin_arch}. 

The source code of the system is available,\footnote{\url{https://github.com/sevagh/MiXiN}} as are sound samples for some separations.\footnote{\url{https://sevag.xyz/music_separation.html}} This name was selected since source separation is also called ``unmixing'' (which is the origin of the name Open-Unmix), and also as a backcronym for the letters MXN, for \textbf{M}usic e\textbf{X}traction with \textbf{N}onstationary Gabor Transforms.

\begin{figure}[ht]
	\centering
	\includegraphics[width=14cm]{./mixin_arch.png}
	\caption{MiXiN architecture}
	\label{fig:mixin_arch}
\end{figure}

The data used to train MiXiN were stems from 4 albums from the progressive metal band Periphery, purchased from their merchandise website.\footnote{\url{https://store.periphery.net/music/music}} The instruments were split as follows -- the drum stem was used for the percussive source, the vocal stem for the vocal source, and a sum of all other sources (rhythm + lead guitar + misc.) for the harmonic source. The data was split into 80\%/20\%/20\% test/train/validation (this is about 128 minutes of total training music). The training curves for the 3 CDAEs (harmonic, percussive, and vocal) are shown in figure \ref{fig:trainingcurves}. The model was implemented using Tensorflow \cite{tensorflow} and Keras \cite{keras}.

The training loss chosen was mean absolute error (MAE) between the input magnitude NSGT of the mixed source and output magnitude NSGT of the desired target. The training data was split into 1-second chunks, and the magnitude NSGT with 96 bins on the Bark scale between 0 and 22050 Hz was taken, creating training data with an input of $1948 \times 96$ for 1 second of mixed audio and an output of $1948 \times 96$ for 1 second of source audio.

\begin{figure}[ht]
	\centering
	\subfloat[Harmonic loss]{{\includegraphics[width=6cm]{./harmonic_train_loss.png} }}
	\subfloat[Percussive loss]{{\includegraphics[width=6cm]{./percussive_train_loss.png} }}
	\subfloat[Vocal loss]{{\includegraphics[width=6cm]{./vocal_train_loss.png} }}
	\caption{Training curves for the MAE loss of the 3 CDAEs}
	\label{fig:trainingcurves}
\end{figure}

Finally, the BSSv4 results for MiXiN are shown in figure \ref{fig:bssv4mixin}.

\begin{figure}[ht]
	\centering
	\makebox[\textwidth]{\includegraphics[width=16cm]{mixin_bssv4.png}}
	\caption{Final HPSS results, BSSv4 scores}
	\label{fig:bssv4mixin}
\end{figure}

It does not score better than Open-Unmix, but it does show that STFT-based deep learning models can be applied to the NSGT. A useful comparison could have been to compare the STFT-based CDAE architecture to the NSGT-based CDAE architecture, to verify whether the NSGT improves the results in some way. However, the original CDAE paper \cite{grais2017single} did not provide a code implementation, and I couldn't successfully create an STFT-based model which sounded as good as the presented NSGT version.

MiXiN has some additional flexibility -- it can be configured to run in instrumental mode (i.e., only perform harmonic/percussive source separation) by omitting the vocal CDAE. This performs better on instrumental music.
...
\begin{tabular}{ |l|l|l| }
	 ...
	 \hline
	 MiXiN & \Verb#vendor/MiXiN# & Git submodule \\
	 \hline
\end{tabular}
...
\begin{tabular}{ |l|l|l| }
	 ...
	 \hline
	 Realtime HPSS STFT (sec \ref{sec:rthpss}) & \Verb#algorithms/HPSS_STFT_realtime.m# & MATLAB \\
	 \hline
	 Realtime HPSS sliCQ (sec \ref{sec:rthpss}) & \Verb#algorithms/HPSS_CQNSGT_realtime.py# & Python \\
	 ...
	 \hline
	 MiXiN MATLAB shim & \Verb#algorithms/MiXiN_python_shim.m# & MATLAB \\
	 \hline
\end{tabular}
...
\begin{tabular}{ |l|l| }
	 ...
	 \Verb#evaluation/contenders/realtime.m# & Realtime HPSS (sec \ref{sec:rthpss}) \\
	 ...
	 \Verb#evaluation/contenders/neural_hpss.m# & Open-Unmix + MiXiN (sec \ref{appendix:ml}) \\
	 \Verb#evaluation/contenders/neural_vocal.m# & Open-Unmix + MiXiN (sec \ref{appendix:ml}) \\
	 \hline

...
\citet{gabor1946}'s seminal signal processing paper, \textit{The Theory of Communication}, contained the first suggestion of time-frequency decomposition of a signal by applying the Fourier transform locally to overlapping portions of the signal multiplied by Gaussian windows. This can be called the 1-D Gabor dictionary:
\[ \mathcal{G} = \{\phi_{n, m}(x) = w(x - \beta m)e^{j2\pi \alpha n x}\}_{n,m \in \mathbb{Z}}, \]

where $w(\cdot)$ is a Gaussian low-pass window function localized at 0, and $\alpha$ and $\beta$ control the time and frequency resolution of the transform. With his transform, Gabor also introduced the first formulation of the time-frequency uncertainty principle, stating that ``although we can carry out the analysis [of the acoustic signal] with any degree of accuracy in the time direction or frequency direction, we cannot carry it out simultaneously in both beyond a certain limit.'' Mathematically, this can be stated as:
\[ \sigma_{t}\sigma_{f} \ge \frac{1}{4\pi}, \Delta t\Delta f \ge 1 \]

The values $\sigma_{t,f}$ represent the standard deviation, while $\Delta_{t,f}$ are, as defined by Gabor, ``the uncertainties inherent in the definition of the epoch $t$ and frequency $f$ of an oscillation.'' This is further illustrated in figure \ref{fig:gabortf}, which shows the tiling of the time-frequency plane, and how the width of the Gaussian can be adjusted to change the time-frequency resolution.

\begin{figure}[ht]
	\centering
	\subfloat[Maximum signal localization in TF: $logon$ with a lower bound area of $\Delta t\Delta f = 1$]{\includegraphics[height=4.5cm]{./gabor1.png}}
	\hspace{2.5em}
	\subfloat[Modifying TF resolution with the Gaussian window shape]{\includegraphics[height=4.5cm]{./gabor2.png}}
	\caption{Gabor's time-frequency uncertainty principal, and the \textit{logon}, or basic unit of time-frequency information}
	\label{fig:gabortf}
\end{figure}

The TF uncertainty principle arises from the fact that time and frequency are, in quantum physics terms, conjugate variables, or Fourier transforms of each other, and from the fact that the Fourier transform is a linear operator. Psychoacoustic research shows that humans have been able to beat the time-frequency uncertainty principle \cite{psycho1, psycho2}, indicating the presence of nonlinear operators in the auditory system.

Gabor similarly noted the need for variable-frequency analysis, stating that ``the foregoing solutions [of the Fourier transform], though unquestionably mathematically correct, are somewhat difficult to reconcile with our physical intuitions and our physical concepts of such variable frequency mechanisms as, for instance, the siren.''

The STFT, or short-time Fourier transform, has been described independently from Gabor's work \cite{stftindie}, but additional research in the 1980s \cite{dictionary} led to the STFT being formalized and described as a special case of the Gabor transform, in recognition of Gabor's pioneering work.

DESCRIBE STFT HERE hann window overlap etc.
...
Soft masks generally produce higher quality sound, as shown in figure \ref{fig:masks}. The oracle mask is the ideal mask for a given signal -- to compute it, the target signal must be known. 

FLESH THIS OUT! \textbf{\textcolor{red}{TODO }}
...
\subsubsection{\textbf{\textcolor{red}{TODO }}Median filtering as matching pursuit}
...
\subsubsection{\textbf{\textcolor{red}{TODO }}Realtime algorithms}
...
\begin{appendices}
\section{\textbf{\textcolor{red}{TODO }}Open-Unmix}

describe how open-unmix works

\section{\textbf{\textcolor{red}{TODO }}MiXiN}

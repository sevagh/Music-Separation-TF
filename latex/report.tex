...
where $w(\cdot)$ is a Gaussian low-pass window function localized at 0, and $\alpha$ and $\beta$ control the time and frequency resolution of the transform. With his transform, Gabor also introduced the first formulation of the time-frequency uncertainty principle, stating that ``although we can carry out the analysis [of the acoustic signal] with any degree of accuracy in the time direction or frequency direction, we cannot carry it out simultaneously in both beyond a certain limit.'' This is illustrated in figure \ref{fig:gabortf}, which also shows how the width of the Gaussian affects the time-frequency resolution.

\begin{figure}[ht]
	\centering
	\subfloat[Max signal localization in TF: rectangles of size $\Delta t\Delta f = 1$]{\includegraphics[height=4.5cm]{./gabor1.png}}
	\hspace{2.5em}
	\subfloat[Change TF tiling by modifying Gaussian]{\includegraphics[height=4.5cm]{./gabor2.png}}
	\caption{Gabor's time-frequency uncertainty principal, and the \textit{logon}, or basic unit of time-frequency information}
	\label{fig:gabortf}
\end{figure}

The TF uncertainty principle arises from the fact that time and frequency are, in quantum physics terms, conjugate variables, or Fourier transforms of each other, and from the fact that the Fourier transform is a linear operator. Gabor himself came to the same conclusion, stating that ``the foregoing solutions [of the Fourier transform], though unquestionably mathematiccally correct, are somewhat difficult to reconcile with our physical intuitions and our physical concepts of such variable frequency mechanisms as, for instance, the siren.''

Moreover, psychoacoustic research shows that humans have been able to beat the time-frequency uncertainty principle \cite{psycho1, psycho2}, indicating the presence of nonlinear operators in the auditory system. As such, wavelet approaches using nonlinear atoms to beat this limit are an area of active research.

The STFT, or short-time Fourier transform, was independently discovered \cite{stftindie}, and \citet{dictionary} describes that additional work in the 1980s led to the STFT being described as a special case of the Gabor transform, in homage to Gabor's pioneering work.

\citet{dictionary} also discuss the shift from the term \textit{transform}, e.g., the Gabor transform or STFT, to \textit{dictionary}, stating that works by \cite{dictionary1} and \cite{dictionary2} began the ``fundamental shift from transforms to dictionaries for sparse signal representation.'' Accordingly, an important outcome of this terminology change was the ``idea that a signal was allowed to have more than one description in the representation domain, and that selecting the best one depended on the task.'' Similar ideas were shown in \citet{doerflerphd}'s dissertation, suggesting the use of multiple Gabor dictionaries for representing music, for the two following reasons:

\begin{tight_itemize}
	\item
		\textit{Transients} are important musically, driving instrument identification and temporal events such as beats. As transients are in the higher frequency range, good time resolution in that range allows clearer identification of transient events.
	\item
		 The low frequencies of the bass instruments lay the harmonic basis of the song, requiring good frequency resolution at lower frequencies.
\end{tight_itemize}

The idea for a musically-appropriate transform, the Constant-Q Transform was first proposed by \citet{jbrown}. B

...
Several important outcomes of the NSGT were the invertible CQT with a fast FFT-based implementation \cite{invertiblecqt}, and the realtime invertible CQT \cite{rtcqt}. Aside from being available in LTFAT, the official MATLAB Wavelet Toolbox contains perfectly invertible implementations of the CQT and ICQT (inverse CQT) based on the NSGT \cite{matlabcqt}; these transforms are used extensively throughout this paper, present in every MATLAB algorithm which uses the CQT. In the Python case, the NSGT library is used \cite{nsgtpy}, which contains both the invertible and realtime implementations \cite{invertiblecqt, rtcqt}.

\subsection{\textbf{\textcolor{red}{TODO }}Representations of tonal and transient sounds}

After describing the theoretical history of these various signal transforms, a useful exercise would be to represent the classic glockenspeil signal with different dictionaries to gain some practical, hands-on experience with the characteristics, advantages, and disadvantages of each transform.

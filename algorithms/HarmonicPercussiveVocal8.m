function HarmonicPercussiveVocal8(filename, varargin)
...
for bands = 1:1:size(hzVect, 2)-1
    ...
    yp = bandpass(xp, bandEdges, fs);

    [~, ~, attack, ~] = transientShaper(yp, fs,...
        attackFastMs, attackSlowMs, releaseMs);

    yp_enhanced = yp .* attack;

    xh_suppressed = xh_suppressed + yh_suppressed;
    xp_enhanced = xp_enhanced + yp_enhanced;
end

audiowrite(xhOut, xh_suppressed, fs);
audiowrite(xpOut, xp_enhanced, fs);

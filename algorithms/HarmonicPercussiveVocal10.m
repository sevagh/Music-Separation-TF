function HarmonicPercussiveVocal10(filename, varargin)
addpath(genpath('../vendor/multiband-transient-shaper'));
...
for bands = 1:1:size(hzVect, 2)-1
    ...
    [~, ~, ~, sustainh] = transientShaper(yh, fs,...
        attackFastMs, attackSlowMs, releaseMs);

    yh_suppressed = yh .* sustainh;
    
    yv = bandpass(xv, bandEdges, fs);

    [~, ~, ~, sustainv] = transientShaper(yv, fs,...
        attackFastMs, attackSlowMs, releaseMs);

    yv_suppressed = yv .* sustainv;

    xh_suppressed = xh_suppressed + yh_suppressed;
    xv_suppressed = xv_suppressed + yv_suppressed;
end

audiowrite(xhOut, xh_suppressed, fs);
audiowrite(xpOut, xp3+xp2, fs);
audiowrite(xvOut, xv_suppressed, fs);
end
